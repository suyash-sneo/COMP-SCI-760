...
class QLearning(AbstractSolver):
    ...
    def train_episode(self):
        ...
        for _ in range(self.options.steps):
            ...
            self.Q[state][action] = ((1 - self.options.alpha)*self.Q[state][action]) + (self.options.alpha * (reward + (self.options.gamma*q_max)))
            state = next_state
            if done:
                break
    ...
    def epsilon_greedy_action(self, state):
        ...
        if rv <= self.options.epsilon:
            choice = np.random.choice(np.arange(start=0, stop=self.env.action_space.n, step=1))
            print("random choice:", choice)
            return choice
        else:
            return self.create_greedy_policy()(state=state)
