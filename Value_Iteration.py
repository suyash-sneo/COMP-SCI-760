        ...
        for each_state in range(self.env.nS):
            ...
            for action in range(self.env.nA):
                for probability, next_state, reward, _ in self.env.P[each_state][action]:
                    action_vals[action] += (probability * (reward + (self.options.gamma*self.V[next_state])))
            
            temp_state[each_state] = np.max(action_vals)
